...
class OneAU():
    def __init__(self, DFAU):
        self.DFAU = DFAU  ## directory dataframe Australia dataset
    ...
    def normalizedDF(self):
        DF = pd.read_csv(self.pwd + self.sep + self.DFAU)
...
class lstmTAS():
    def __init__(self, DFTAS):
        self.DFTAS = pd.read_csv(DFTAS)  ## directory dataframe Australia dataset
        self.stations = self.DFTAS['site_id'].unique().tolist()
    ...
    def run_lstmTAS(self):
        for i, sta in enumerate(self.stations):
            num_loss =1
            while (num_loss >= 0.9):
                model, test, DFvali, DFtest, num_loss = self.LOOCV(sta, 0.003, 0.8, 100, 1e-08)
                print(num_loss)
